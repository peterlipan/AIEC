...
class RandomFeatureMasking(RandomFeatureAugmentation):
    def __init__(self, p=1, max_mask_ratio=0.1):
        super().__init__(p)
        self.max_mask_ratio = max_mask_ratio

    def augment(self, x):
        dim = x.shape[-1]
        mask_len = int(dim * self.max_mask_ratio)
        start = random.randint(0, dim - mask_len)
        x[..., start:start + mask_len] = 0
        return x


class RandomFourierFeatureNoise(RandomFeatureAugmentation):
    ...
    def augment(self, x):
        ...
        noise = torch.randn_like(fft) * self.std
        noisy_fft = fft + noise
        return torch.fft.ifft(noisy_fft, dim=-1).real.type_as(x)


class RandomFrequencyDropout(RandomFeatureAugmentation):
    def __init__(self, p=1, max_drop_ratio=0.2):
        super().__init__(p)
    ...
    def augment(self, x):
        fft = torch.fft.fft(x.float(), dim=-1)
        N = x.shape[-1]
        drop_len = int(N * self.max_drop_ratio)
        start = random.randint(0, N - drop_len)
        fft[..., start:start + drop_len] = 0
        return torch.fft.ifft(fft, dim=-1).real.type_as(x)


class RandomLowFreqBias(RandomFeatureAugmentation):
    def __init__(self, p=1, bias_strength=0.2):
        super().__init__(p)
    ...
    def augment(self, x):
        fft = torch.fft.fft(x.float(), dim=-1)
        N = fft.shape[-1]
        freq_weights = torch.linspace(1.0, 0.5, N, device=x.device)
        fft = fft * (1 + self.bias_strength * freq_weights)
        return torch.fft.ifft(fft, dim=-1).real.type_as(x)


class RandomPhaseShuffle(RandomFeatureAugmentation):
    def __init__(self, p=1, max_shift=10):
        super().__init__(p)
    ...
    def augment(self, x):
        ...
        shuffled_phase = torch.roll(phase, shifts=shift, dims=-1)
        fft_mag = torch.abs(fft)
        new_fft = fft_mag * torch.exp(1j * shuffled_phase)
        return torch.fft.ifft(new_fft, dim=-1).real.type_as(x)


class RandomBandNoise(RandomFeatureAugmentation):
    def __init__(self, p=1, band=(0.3, 0.6), std=0.1):
        super().__init__(p)
        self.band = band
        self.std = std
    ...
    def augment(self, x):
        fft = torch.fft.fft(x.float(), dim=-1)
        N = x.shape[-1]
        start = int(self.band[0] * N)
        end = int(self.band[1] * N)
        noise = torch.randn(end - start, device=x.device) * self.std
        fft[..., start:end] += noise
        return torch.fft.ifft(fft, dim=-1).real.type_as(x)


class RandomFeaturePermutation(RandomFeatureAugmentation):
    def __init__(self, p=1):
        super().__init__(p)
...
def get_train_transforms(n_experts, num_levels, downsample_factor, lowest_level, dropout, visible_levels):
    ...
    feature_augmentations = Compose([
        ...
        OneOf([
            RandomGaussianNoise(p=1, std=0.05),
            RandomFeatureScaling(p=1, scale_range=(0.8, 1.2)),
            RandomFeatureShift(p=1, shift_std=0.05),
        ], p=0.5),

        OneOf([
            RandomFeatureDropout(p=1, dropout_rate=0.2),
            RandomFeatureMasking(p=1, max_mask_ratio=0.1),
        ], p=0.5),

        # Frequency-domain augmentations
        OneOf([
            RandomFourierFeatureNoise(p=1, std=0.05),
            RandomFrequencyDropout(p=1, max_drop_ratio=0.2),
            RandomLowFreqBias(p=1, bias_strength=0.15),
            RandomPhaseShuffle(p=1, max_shift=20),
            RandomBandNoise(p=1, band=(0.2, 0.6), std=0.05),
        ], p=0.5),
    ])
